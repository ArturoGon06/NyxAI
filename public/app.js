...
const defaultProvider = { active_provider: "ollama", ollama_base_url: null, selected_model: null, character_creator_model: null };
...
const defaultMemorySettings = { enabled: false, automatic_extraction: true, embedding_model: null, extraction_model: null, extraction_interval: 8, retrieval_count: 4, similarity_threshold: .62, context_budget: 512 };
...
function showSettings() { closeDrawer(); showScreen(settingsScreen); document.querySelector("#close-settings").focus(); void loadSettings().then(() => Promise.all([loadModels({ quiet: true }), loadImageModels({ quiet: true })])); }
...
function applySettings(settings) {
  ...
  renderPersonaCards();
  document.querySelector("#ollama-url").value = appSettings.provider.ollama_base_url || appSettings.effective_ollama_base_url;
...
function setConnectionState(state) {
  ...
  connectionDot.setAttribute("aria-label", state === "connected" ? "Connected to Ollama" : state === "connecting" ? "Connecting to Ollama" : state === "error" ? "Ollama connection failed" : "Connection not tested");
}
async function saveConnection({ testAfterSave = false } = {}) { appSettings.provider.ollama_base_url = document.querySelector("#ollama-url").value.trim() || null; showStatus(connectionStatus, "Saving server…"); await persistSettings(); showStatus(connectionStatus, "Ollama server saved.", "success"); if (testAfterSave) await testConnection(); }
async function testConnection() { setConnectionState("connecting"); showStatus(connectionStatus, "Connecting to Ollama…"); try { const result = await readJson(await fetch("/api/providers/ollama/test", { method: "POST" }), "Could not connect to Ollama."); setConnectionState("connected"); showStatus(connectionStatus, result.message, "success"); await loadModels({ quiet: true }); } catch (error) { setConnectionState("error"); showStatus(connectionStatus, userFacingNetworkError(error, "Could not connect to Ollama."), "error"); } }
...
function populateCharacterCreatorModelChoices(models = availableModels) {
  ...
  if (!hasModels) showStatus(characterCreatorModelStatus, "Connect to Ollama and refresh models to choose a creator model.");
...
function populateModels(result) {
  modelSelect.replaceChildren(); modelSelect.add(new Option(result.models.length ? "Choose a model" : "No models installed in Ollama", ""));
  if (!result.models.length) { modelSelect.disabled = true; document.querySelector("#save-model").disabled = true; populateCharacterCreatorModelChoices([]); showStatus(modelStatus, "Ollama is connected, but no local models are installed yet."); return; }
  ...
  modelSelect.disabled = false; document.querySelector("#save-model").disabled = !modelSelect.value;
...
async function loadModels({ quiet = false } = {}) { if (!quiet) showStatus(modelStatus, "Refreshing local models…"); try { const result = await readJson(await fetch("/api/providers/ollama/models", { method: "POST" }), "Models could not be retrieved."); populateModels(result); if (!quiet) setConnectionState("connected"); } catch (error) { modelSelect.replaceChildren(new Option("Models unavailable", "")); modelSelect.disabled = true; document.querySelector("#save-model").disabled = true; populateCharacterCreatorModelChoices([]); showStatus(modelStatus, userFacingNetworkError(error, "Models could not be retrieved."), "error"); if (!quiet) setConnectionState("error"); } }
async function selectModel() { const model = modelSelect.value; if (!model) { showStatus(modelStatus, "Choose a model first.", "error"); return; } document.querySelector("#save-model").disabled = true; showStatus(modelStatus, "Saving selected model…"); try { const result = await readJson(await fetch("/api/providers/ollama/model", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model }) }), "The model could not be selected."); appSettings.provider.selected_model = result.selected_model; populateModels(result); } catch (error) { showStatus(modelStatus, error.message, "error"); document.querySelector("#save-model").disabled = false; } }
...
function populateMemoryModelChoices() {
  const embedding = document.querySelector("#memory-embedding-model"); const extraction = document.querySelector("#memory-extraction-model");
  embedding.replaceChildren(new Option(availableModels.length ? "Choose an embedding model" : "Refresh Ollama models first", ""));
  extraction.replaceChildren(new Option("Use Character Creator or chat model", ""));
  availableModels.forEach((model) => { embedding.add(new Option(formatModel(model), model.name)); extraction.add(new Option(formatModel(model), model.name)); });
  embedding.value = availableModels.some((model) => model.name === appSettings.memory.embedding_model) ? appSettings.memory.embedding_model : "";
  ...
  embedding.disabled = !availableModels.length; extraction.disabled = !availableModels.length;
}
...
function memorySettingsFromControls() {
  return { enabled: document.querySelector("#memory-enabled").checked, automatic_extraction: document.querySelector("#memory-automatic").checked, embedding_model: document.querySelector("#memory-embedding-model").value || null, extraction_model: document.querySelector("#memory-extraction-model").value || null, extraction_interval: Number(document.querySelector("#memory-interval").value), retrieval_count: Number(document.querySelector("#memory-retrieval-count").value), similarity_threshold: Number(document.querySelector("#memory-threshold").value), context_budget: Number(document.querySelector("#memory-budget").value) };
}
async function saveMemorySettings() {
  ...
  try { await persistSettings(); showStatus(status, previous.embedding_model && previous.embedding_model !== appSettings.memory.embedding_model ? "Embedding model saved. Rebuild the memory index before retrieval resumes." : "Semantic memory settings saved.", "success"); }
...
async function sendMessage(message) {
  ...
  if (!appSettings.provider.selected_model) { showChatStatus("Choose an Ollama model in Settings before sending a message.", "error"); showSettings(); return; }
  ...
  try {
    ...
    if (!receivedContent) { renderRoleplayContent(assistantTarget.content, "No response was generated."); showChatStatus("Ollama completed without returning text.", "error"); } else showChatStatus("");
  } catch (error) {
...
modelSelect.addEventListener("change", () => { document.querySelector("#save-model").disabled = !modelSelect.value; if (modelSelect.value) showStatus(modelStatus, "Save this selection to use it."); });
...
stopButton.addEventListener("click", () => { if (activeController) { showChatStatus("Stopping generation…"); activeController.abort(); } });
